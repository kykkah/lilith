...
class ShadowCopyAction implements CopyAction {
    ...
    private static FilePermissions fromUnixMode(int unixMode) {
        int perms = unixMode & 0777
        return new FilePermissions() {
            private final UserClassFilePermissions user = new ImmutableUserClassFilePermissions((perms >> 6) & 0b111)
            private final UserClassFilePermissions group = new ImmutableUserClassFilePermissions((perms >> 3) & 0b111)
            private final UserClassFilePermissions other = new ImmutableUserClassFilePermissions(perms & 0b111)

            @Override
            UserClassFilePermissions getUser() { return user }

            @Override
            UserClassFilePermissions getGroup() { return group }

            @Override
            UserClassFilePermissions getOther() { return other }

            @Override
            int toUnixNumeric() { return perms }
        }
    }

    private static class ImmutableUserClassFilePermissions implements UserClassFilePermissions {
        private final boolean read
        private final boolean write
        private final boolean execute

        ImmutableUserClassFilePermissions(int bits) {
            this.read = (bits & 0b100) != 0
            this.write = (bits & 0b010) != 0
            this.execute = (bits & 0b001) != 0
        }
        ...
        @Override
        boolean getWrite() { return write }

        @Override
        boolean getExecute() { return execute }
    }
    ...
    class ArchiveFileTreeElement implements FileTreeElement {
        ...
        FilePermissions getPermissions() {
            return fromUnixMode(archivePath.entry.unixMode)
        }
